...
Retrieves relevant chunks from the Siddha Pinecone namespace + BM25 index
using hybrid RRF, then calls the LLM with a Siddha-specific prompt.
"""
...
from app.prompts.siddha_prompt import SIDDHA_SYSTEM, SIDDHA_USER
...
async def run_siddha_expert(
    query: str,
    chunks: list[RetrievedChunk] | None = None,
    history: list[dict] | None = None,
) -> tuple[Optional[ExpertResponse], list[RetrievedChunk]]:
    """Run the Siddha expert agent."""
    log.info("siddha_expert_start", query=query[:80])
    ...
    if not context.strip():
        log.warning("siddha_no_context", query=query[:80])
        context = "No specific documents were retrieved. Use your comprehensive Siddha medical knowledge."

    messages = [
        SystemMessage(content=SIDDHA_SYSTEM),
        HumanMessage(content=SIDDHA_USER.format(query=query, context=context, history=history_text)),
    ]

    try:
        response = await call_llm(
            messages,
            model=settings.siddha_model,
        ...
        log.error("siddha_json_error", error=str(exc))
        return ExpertResponse(diagnosis="Siddha expert encountered a parsing error.", confidence=0.0), chunks
    except Exception as exc:
        log.error("siddha_expert_error", error=str(exc))
        raise
